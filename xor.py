...
class Xor():
   ...
   def encrypt(self, doc, output, key_out, entropy):
      ...
      #print('getFile', 'getKey')
      #print('self.getEntropy(key_out)')
      if self.getEntropy(key_out) > entropy:

         xor_in = self.xorOperation(getFile.decode(), getKey)
         #print('self.xorOperation(getFile.decode(), getKey)')
   ...
   def decrypt(self, doc, key, decrypted_file):
      ...
      '''
      #print('decrypt')
      getFile = self.read(doc)
      #print(getFile)
      readKey = self.readKey(key)
      #print(readKey)
      xor_out = self.xorOperation(getFile.decode(), readKey.decode())
      #print(xor_out)
   ...
   def getEntropy(self, entry):
      ...
      getSizeFun = self.getSize(entry)
      getSize = getSizeFun[0]
      byteArray = getSizeFun[1]
      freqList = []
      for b in range(256):
          ctr = 0
   ...
   def madEncrypt(self, doc, encryptedDoc, key_out, entropy, RubbishOutput, n, chunkSize, division_name, chunks, tarIN, tarName, tarkey):
      ...
      print('Starting Tar encrypt')
      self.encrypt(tarIN, tarName, tarkey, entropy)
      print('Tar encrypt End')
